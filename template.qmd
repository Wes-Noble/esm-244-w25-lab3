...
1) What is your research Questions
2) Gather data
3) Look at your data! Exploratory Data Analysis
4) Develop a Hypothesis
5) Choosing metrics (what metrics are we going to use)
6) Data split for testing and training
7) Building a model:
  a) Create model options, what variables
  b) data preproccessing
  c) train the model
  d) tune the model
  e) evaluate the performance
  f) model selection
8) Finalize the model (aka train on whole dataset)
9) interpret results/visualize/communicate
...
1) I would want to clean the names of the people
2) maybe make some stuff a factor -- like sex or embarked
3) think about what variables we want to inlcude
...
ggplot(t_df, aes(x = pclass, fill = as.factor(survived))) +
  geom_bar(position = "dodge") +
  labs(title = "Survival by Class", x = "Class", y = "Count", fill = "Survived") +
  theme_minimal()
...
lasso_rec<-recipe(survived~.,data=surv_train_df) %>%
  update_role(passenger_id, new_role = "ID") %>% # telling the it is just a preset for ID
  step_rm(name,age) %>% # getting rid of name and age since there are so many NAs
  step_unknown(all_nominal(),-all_outcomes()) |> 
  step_dummy(all_nominal(),-all_outcomes()) |> # all category variables are dummy variables
  step_zv(all_numeric(),-all_outcomes()) |> # get did of 0 variance variables
  step_normalize(all_numeric(),-all_outcomes()) # all numeric data to be normalized
